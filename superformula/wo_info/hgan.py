...
#        optimizer = RMSprop(lr=1e-3)
    ...
    def train(self, train_steps=2000, batch_size=256, save_interval=0, mode='startover'):
        ...
        g1_fname = '../hgan_idetc2018_data/superformula/wo_info/generator1.h5'
        g2_fname = '../hgan_idetc2018_data/superformula/wo_info/generator2.h5'
        d_fname = '../hgan_idetc2018_data/superformula/wo_info/discriminator.h5'
...
#                y_fake = label_flipping(y_fake, .1)
                ...
                if save_interval>0 and (t+1)%save_interval==0:
                    ...
                    plot_grid(9, gen_func=self.synthesize_parent, d=self.latent_dim,
                              scale=.85, save_path='superformula/wo_info/parent.svg')
                    plot_grid(9, gen_func=self.synthesize_child, d=self.latent_dim,
                              scale=.85, save_path='superformula/wo_info/child.svg')
...
#            superformula = self.x_test[6, :self.x_test.shape[1]/2]
        N = c2.shape[0]
        noise2 = np.random.normal(scale=0.5, size=(N, self.noise_dim))
        parent = np.tile(superformula, (N,1,1,1))
